...
class MainWindow(QWidget):
    ...
    def create_label_second(self, main_w = 1, main_h = 1):

        self.label_second.setScaledContents(True)
        self.label_second.move(30, 450)
        self.label_second.resize(1100, 350)
        self.label_second.setStyleSheet("background-color:  rgb(77,77,77)")
        print(f'Размеры во втором виджете{main_w},{main_h}')

        self.label_second.show()
    ...
    def resizeEvent(self, event):
        w = event.size().width()
        h = event.size().height()
        self.label_second.resize(w / 2, h / 2)

        pixmap = self.label_second.pixmap()
        pixmap = pixmap.scaled(w / 2, h / 2)  # масштабирование

        self.label_second.setPixmap(pixmap)  # применение
        print(w, h)
